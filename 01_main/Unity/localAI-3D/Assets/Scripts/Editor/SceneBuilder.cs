...
public class SceneBuilder
{
    ...
    // ===== 3D Model + Floor 統合 =====
    static void CreateFloorAndCharacter()
    {
        // ===== AutonomousWorld =====
        var autonomousWorld = new GameObject("AutonomousWorld");
        autonomousWorld.AddComponent<AutonomousPlayBootstrap>();

        // ===== Camera =====
        var cameraObj = new GameObject("Main Camera");
        cameraObj.transform.SetParent(autonomousWorld.transform);
        var camera = cameraObj.AddComponent<Camera>();
        camera.backgroundColor = new Color(0.15f, 0.15f, 0.2f);
        cameraObj.tag = "MainCamera";
        cameraObj.AddComponent<AudioListener>();
        // Chat UI に合わせたカメラ位置（下側）
        cameraObj.transform.position = new Vector3(0, 3, -8);
        cameraObj.transform.rotation = Quaternion.Euler(22, 0, 0);

        // ===== Floor（Chat UI の横幅に合わせて 1080 units） =====
        var floorObj = new GameObject("WalkFloor");
        floorObj.transform.SetParent(autonomousWorld.transform);
        var floor = floorObj.AddComponent<MeshFilter>();
        var floorRenderer = floorObj.AddComponent<MeshRenderer>();
        floorObj.AddComponent<BoxCollider>();

        // Plane プリミティブを作成
        var planeMesh = Resources.GetBuiltinResource<Mesh>("Plane.fbx");
        floor.mesh = planeMesh;

        // スケール: 横幅 1080（UI 参照解像度）、奥行き 540、高さは薄い
        floorObj.transform.localScale = new Vector3(108f, 0.1f, 54f);
        ...
        // ===== Waypoints（Chat UI の横幅に合わせて分散） =====
        var waypointManagerObj = new GameObject("WaypointManager");
        waypointManagerObj.transform.SetParent(autonomousWorld.transform);
        var waypointManager = waypointManagerObj.AddComponent<WaypointManager>();

        // 6個のウェイポイント（横幅 1080 に合わせて -400 ～ 400）
        ...
        foreach (var waypointPos in waypoints)
        {
            var wpObj = new GameObject("Waypoint");
            wpObj.transform.SetParent(waypointManagerObj.transform);
            wpObj.transform.position = waypointPos;
            var wp = wpObj.AddComponent<Waypoint>();
        }

        // ===== Character（10倍スケール）=====
        var characterObj = new GameObject("Character");
        characterObj.transform.SetParent(autonomousWorld.transform);
        characterObj.transform.position = new Vector3(0, 0, 0);

        // FBX モデルをロード
        var modelPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/untitled.fbx");
        if (modelPrefab != null)
        {
            ...
            if (model != null)
            {
                ...
                // キャラクターコンポーネント追加
                characterObj.AddComponent<CharacterStateMachine>();
                var agent = characterObj.AddComponent<UnityEngine.AI.NavMeshAgent>();
                agent.speed = 2f;
                ...
                characterObj.AddComponent<MovementController>();
                characterObj.AddComponent<CharacterBrain>();
                characterObj.AddComponent<CharacterBehaviorStarter>();

                // Material 修正
                RepairMaterialsInEditorMode(model);

                Debug.Log("[SceneBuilder] Character モデル ロード済み（10倍スケール）");
            }
        }
        else
        {
            Debug.LogWarning("[SceneBuilder] FBX モデル 'Assets/untitled.fbx' が見つかりません");
        }

        // ===== NavMesh は Play 時に自動構築 =====
        Debug.Log("[SceneBuilder] Floor と Waypoints を配置しました。NavMesh は Play 時に自動構築されます");
    }

    // ===== Material 修正（FBX からロード）=====
    static void RepairMaterialsInEditorMode(GameObject model)
    {
        var allAssets = AssetDatabase.LoadAllAssetsAtPath("Assets/untitled.fbx");
        var mats = System.Array.FindAll(allAssets, x => x is Material);
        ...
        if (mats.Length > 0)
        {
            foreach (var renderer in model.GetComponentsInChildren<Renderer>())
            {
                if (mats.Length > 0)
                {
                    var mat = mats[0] as Material;
                    if (mat != null)
                    {
                        renderer.sharedMaterial = mat;
                        renderer.sharedMaterial.SetFloat("_Metallic", 0.1f);
                        renderer.sharedMaterial.SetFloat("_Glossiness", 0.4f);
                    }
                }
            }
            Debug.Log("[SceneBuilder] Material を FBX からロードして適用しました");
        }
        else
        {
            Debug.LogWarning("[SceneBuilder] FBX 内に Material が見つかりません");
        }
    }
}
#endif
